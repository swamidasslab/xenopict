...
def create_elk_graph(nodes: Dict[str, etree._Element], edges: Dict[str, Tuple[str, str]]) -> Dict[str, Any]:
    """
...
def render_layout_svg(layout_result: Dict[str, Any], 
                     nodes: Dict[str, etree._Element],
                     padding: float = 10.0) -> etree._Element:
    """
    ...
    if 'edges' in layout_result:
        edges_group = etree.SubElement(root, 'g', 
                                     attrib={'id': 'edges'},
                                     nsmap=NSMAP)
        for edge in layout_result['edges']:
            if 'sections' in edge:
                ...
                etree.SubElement(edges_group, 'path',
                               attrib={
                                   'd': path_data,
                                   'stroke': "black",
                                   'fill': "none",
    ...
    nodes_group = etree.SubElement(root, 'g', 
                                 attrib={'id': 'nodes'},
                                 nsmap=NSMAP)
    for child in layout_result['children']:
        ...
        if node_id in nodes:
            ...
            node_group = etree.SubElement(nodes_group, 'g',
...
def layout_with_svgs(nodes: Dict[str, etree._Element], 
                    edges: Dict[str, Tuple[str, str]],
                    options: Optional[Dict[str, Any]] = None) -> etree._Element:
    """
