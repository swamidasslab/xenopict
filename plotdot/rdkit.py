...
def shaded_svg(
    ...
    Args:
        ...
        color_map (_type_, optional): Color map. Defaults to plotdot.ColorMap().
...
class ShadingMolDrawer:
    ...
    def __init__(
            self, 
            rdk_mol,
            color_map : ColorMapType = ColorMap(),
        ...
        if compute_coords: rdDepictor.Compute2DCoords(rdk_mol)  # type: ignore
    ...
    def _circle(self,
        ...
        style = style or {}
        if fill is not None: style["fill"] = "rgb(%g,%g,%g)" % tuple(int(x * 255) for x in fill)
        if stroke is not None: style["stroke"] = "rgb(%g,%g,%g)" % tuple(int(x * 255) for x in stroke)
        if style: c.setAttribute("style",_dict2style(style))
    ...
    def _init_mark_layers(self, halo_opacity = "0.5"):
        ...
        self.groups["halo"] = h = self.svgdom.createElementNS('http://www.w3.org/2000/svg', 'g')
